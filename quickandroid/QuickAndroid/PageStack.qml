...
FocusScope {
    ...
    signal myPopped(string pageName)
    ...
    Item {
        ...
        function _realPop(animated) {
            ...
            try {
                ...
                function finished() {
                    //pageStack.poppedPage=poppedPage.objectName;
                    var pageName = poppedPage.objectName;
    ...
    Keys.onReleased: {
        if (event.key === Qt.Key_Back &&
                pages.length > 2) {
            if(topPage.objectName!=='LentaPage'&&topPage.objectName!=='LoginPage'){
                event.accepted = true;
                pop();
            }
        }
